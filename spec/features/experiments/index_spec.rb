...
RSpec.describe "/experiments", type: :feature do
  ...
  describe "as a user, when I visit the experiment index page" do
    ...
    it "displays the names of all long-running experiments in DECENDING order" do
      visit "/experiments"

      expect(@exp_2.name).to appear_before(@exp_5.name)
      expect(@exp_5.name).to appear_before(@exp_3.name)
    end
  end
